...
@pytest.fixture
def mock_b2b_unhappy_response():
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        b2b_url_pattern = re.compile(
                    r"http://localhost:8010/api/v1/products\?ids=.*"
                )
        rsps.add(
...
class TestCreateOrder:
    def test_checkout_creates_paid_order_with_fixed_prices(
        self, jwt_client, test_address, test_payment_method, mock_b2b_happy_response
    ):
        url = reverse("orders")
    ...
    def test_partial_reserve_failure_returns_409(
        self, jwt_client, test_address, test_payment_method, mock_b2b_unhappy_response
    ):
        url = reverse("orders")
